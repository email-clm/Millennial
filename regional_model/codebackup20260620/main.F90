...
do n = 1, ngrid
	...
	!print *, n, current_day, "before LMWC: ", LMWC(current_day, n),  "POMC: ",POM(current_day, n), "MBC: ", MB(current_day, n),  "MINERALC: ",MINERAL(current_day, n),  "AGGC: ",SOILAGG(current_day, n)

	call decomp(soil_par(ID, 1:nbiome), forc_st((current_day),n), forc_sw((current_day),n), psi(n), forc_npp((current_day),n), &
		bulkdensity(n), silt(n), clay(n), LMWC(current_day, n), POM(current_day, n), MB(current_day, n), MINERAL(current_day, n), &
		SOILAGG(current_day, n), f_LM_leaching(current_day, n), f_MI_LM_des(current_day, n),f_LM_MI_sor(current_day, n), &
		f_LM_MB_uptake(current_day, n),f_PO_LM_dep(current_day, n), f_MB_MI_sor(current_day, n), f_PO_SO_agg(current_day, n), &
		f_MI_SO_agg(current_day, n), f_SO_PO_break(current_day, n), f_SO_MI_break(current_day, n),f_MB_atm(current_day, n))

	!print *, n, current_day, "after LMWC: ", LMWC(current_day, n),  "POMC: ",POM(current_day, n), "MBC: ", MB(current_day, n),  "MINERALC: ",MINERAL(current_day, n),  "AGGC: ",SOILAGG(current_day, n)
	...
	!print *, n, current_day, "after LMWC: ", LMWC(current_day, n),  "POMC: ",POM(current_day, n), "MBC: ", MB(current_day, n),  "MINERALC: ",MINERAL(current_day, n),  "AGGC: ",SOILAGG(current_day, n)
	!print *, n, year-startyear+1, yLMWC(year-startyear+1, n),  yPOM(year-startyear+1, n), yMB(year-startyear+1, n), yMINERAL(year-startyear+1, n), ySOILAGG(year-startyear+1, n)
	...
	if(flag_output ==1) then
			...
			!print *, (year-startyear + 1), yLMWC(year-startyear+1, :)
